...
def control_plot(times, controls, filename, ylabel = r"$\eta_D$ [m]", init = False, subtract_optimal_controls = False):
    ...
    if init:
        Y = [opt_control for opt_control in eta_bc_proj[:-int(2*60*60/600)]]
...
def eval_callback(functional_value, value):
    ...
    control_plot(times, value, "controls/init"+str(eval_counter[0])+".pdf", init = True)
...
ocean_elev_func = lambda t: (h_amp * (th_adj.cos(2 * th_adj.pi * (t-(6*3600)) / h_T)) - (h_amp * (th_adj.cos(8 * th_adj.pi * (t-(6*3600)) / h_T))))
...
min_values = th_adj.minimize(rf, options={'gtol':  1e-200, 'ftol': 1e-9, 'maxfun': 1000})


from matplotlib.ticker import MaxNLocator, LogLocator, NullFormatter

ax = plt.figure().gca()
ax.semilogy(max_error_list)

ax.set_xlabel('Iteration')
ax.set_ylabel(r'Maximum $|\eta_{D} - \eta_{exact}|$ (m)')
